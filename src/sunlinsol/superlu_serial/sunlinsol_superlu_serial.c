 ...
 * -----------------------------------------------------------------
 * This is the implementation file for the SuperLU serial implementation
...
#include <sunlinsol/sunlinsol_superlu_serial.h>
...
/*
 * -----------------------------------------------------------------
 * SuperLU serial solver structure accessibility macros:
 * -----------------------------------------------------------------
 */

#define SLU_CONTENT(S)    ( (SUNLinearSolverContent_SuperLU_Serial)(S->content) )
...
SUNLinearSolver SUNLinSol_SuperLU(N_Vector y, SUNMatrix A, SUNContext sunctx)
{
  SUNLinearSolver S;
  SUNLinearSolverContent_SuperLU_Serial content;
  ...
  /* Create content */
  content = NULL;
  content = (SUNLinearSolverContent_SuperLU_Serial) malloc(sizeof *content);
  ...
  /* Allocate content */
  content->perm_r = int32Malloc((int)MatrixRows);
  if (content->perm_r == NULL) { SUNLinSolFree(S); return(NULL); }

  content->perm_c = int32Malloc((int)MatrixRows);
  ...
  xCreate_Dense_Matrix(content->B, (int)MatrixRows, 1, NULL, (int)MatrixRows, SLU_DN,
                       SLU_D, SLU_GE);

  content->X = (SuperMatrix *) malloc(sizeof(SuperMatrix));
  if (content->X == NULL) { SUNLinSolFree(S); return(NULL); }
  content->X->Store = NULL;
  xCreate_Dense_Matrix(content->X, (int)MatrixRows, 1, NULL, (int)MatrixRows, SLU_DN,
                       SLU_D, SLU_GE);

  ETREE(S) = int32Malloc((int)MatrixRows);
  content->R = (realtype*)SUPERLU_MALLOC((size_t)(MatrixRows * sizeof(realtype)));
  content->C = (realtype*)SUPERLU_MALLOC((size_t)(MatrixRows * sizeof(realtype)));
...
{
  return(SUNLINEARSOLVER_SUPERLU_SERIAL);
}
...
int SUNLinSolSetup_SuperLU(SUNLinearSolver S, SUNMatrix A)
{
  ...
  xCreate_CompCol_Matrix(SM_A(S), (int)SUNSparseMatrix_Rows(A),
			             (int)SUNSparseMatrix_Columns(A),
                         (int)SUNSparseMatrix_NNZ(A),
...
int SUNLinSolFree_SuperLU(SUNLinearSolver S)
{
  ...
  if (S->content) {
    ...
    if (PERMR(S)) {
      SUPERLU_FREE(PERMR(S));
      PERMR(S) = NULL;
    }
    if (PERMC(S)) {
      SUPERLU_FREE(PERMC(S));
      PERMC(S) = NULL;
    }
    if (SM_X(S)) {
