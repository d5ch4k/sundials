 ...
 *      Based on cvsRoberts_ASAi_dns.c and modified to use SuperLU
 * -----------------------------------------------------------------
 ...
 * iteration with the SUPERLU linear solver, and a user-supplied
 * Jacobian routine.
...
#include <sunlinsol/sunlinsol_superlu_serial.h> /* access to SuperLU SUNLinearSolver  */
...
int main(int argc, char *argv[])
{
  ...
  LS = SUNLinSol_SuperLU(y, A, sunctx);
...
/* Create sparse SUNMatrix for use in linear solves */
  ...
  /* Create SuperLU SUNLinearSolver object */
  LSB = SUNLinSol_SuperLU(yB, AB, sunctx);
