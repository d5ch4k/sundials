 ...
 * -----------------------------------------------------------------
 * This is the testing routine to check the SUNLinSol SuperLU serial
...
/* ----------------------------------------------------------------------
 * SUNLinSol_SuperLU Linear Solver Testing Routine
 * --------------------------------------------------------------------*/
int main(int argc, char *argv[])
{
  ...
  if (argc < 4){
    printf("ERROR: FOUR (4) Inputs required: matrix size, matrix type (0/1), print timing \n");
    return(-1);
  }
  ...
  printf("\nSuperLU serial linear solver test: size %ld, type %i\n\n",
         (long int) N, mattype);
  ...
  /* Create SuperLU linear solver */
  LS = SUNLinSol_SuperLU(x, A, sunctx);
  ...
  fails += Test_SUNLinSolGetID(LS, SUNLINEARSOLVER_SUPERLU_SERIAL, 0);
